            ...
            try:
                ...
                if not isinstance(channel, discord.DMChannel):
                    ...
                    if not permissions.send_messages or not permissions.embed_links:
                        logger.warning(f'채널 [{channel.name}]에 메시지를 보낼 권한이 없습니다.')
                        continue
                ...
                embed.add_field(
                    name="구분", 
                    value=scrapper_type.get_korean_name(), 
                    inline=True
                )
                ...
                logger.info(f'채널 [{getattr(channel, "name", "DM")}]에 공지사항을 전송했습니다: {notice.title}')
                
            except discord.Forbidden:
                logger.warning(f'채널 [{getattr(channel, "name", "DM")}]에 메시지를 보낼 권한이 없습니다.')
